...
- **`generated_by` frontmatter** — all output templates (general, recipe, x-thread) now stamp a nested `generated_by:` mapping in YAML frontmatter with `skill: https://github.com/cristoslc/media-summary` and `model: <runtime model id>`, so summaries are self-attributing and capture which model wrote them.
